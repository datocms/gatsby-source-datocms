...
const isImage = ({ format, width, height }) => ['png', 'jpg', 'jpeg', 'gif'].includes(format) && width && height;

const createUrl = function () {
  const image = arguments[0];
  const options = decamelizeKeys(Object.assign.apply(null, [{}].concat(Array.prototype.slice.call(arguments, 1))), { separator: '-' });
...
const getBase64ImageAndBasicMeasurements = (image, args) => getBase64Image(image).then(base64Str => {
  let aspectRatio;
...
const resolveResolution = (image, options) => {
  ...
  return getBase64ImageAndBasicMeasurements(image, options).then(({ base64Str, width, height, aspectRatio }) => {
    ...
    // Create the srcSet
    const srcSet = sizes.filter(size => size < width).map((size, i) => {
      let resolution;
      switch (i) {
        case 0:
          resolution = `1x`;
...
const resolveSizes = (image, options) => {
  ...
  return getBase64ImageAndBasicMeasurements(image, options).then(({ base64Str, width, height, aspectRatio }) => {
    ...
    // Create the srcSet.
    const srcSet = filteredSizes.map(width => {
