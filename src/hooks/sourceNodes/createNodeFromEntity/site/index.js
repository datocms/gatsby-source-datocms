...
module.exports = function buildSiteNode(entity, { entitiesRepo, localeFallbacks }) {
  return [].concat(
    ...entity.locales.map((locale) => {
      const additionalNodesToCreate = [];
      const i18n = { locale, fallbacks: localeFallbacks };

      const siteNode = buildNode('DatoCmsSite', `${entity.id}-${locale}`, (node) => {
        node.locale = locale;
        ...
        [
          ...
          'internalDomain',
          'noIndex',
        ].forEach(key => node[key] = entity[key]);
        ...
        if (globalSeo) {
          ...
          if (globalSeo.fallbackSeo) {
            node.globalSeo.fallbackSeo = {
              ...
              image___NODE: node.globalSeo.fallbackSeo.image &&
